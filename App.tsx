...
function SingleBracketGraphic({
  ...
  const bracketStream = matches[0]?.bracket ?? "W";
  const useSimpleColumnLayout = bracketStream === "L";
  ...
  orderedRounds.forEach(([roundNumber, roundMatches], roundIndex) => {
    roundMatches.forEach((match, index) => {
      let centerY = 0;
      if (roundIndex === 0 || useSimpleColumnLayout) {
        // Losers bracket dependency paths are not a simple binary tree, so use stable
        // row spacing instead of trying to center every card on upstream dependencies.
        // This prevents overlap and keeps every match readable.
